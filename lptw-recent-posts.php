...
add_action( 'plugins_loaded','lptw_recent_posts_localisation');
function lptw_recent_posts_localisation(){
	load_plugin_textdomain( 'lptw_recent_posts', false, plugin_basename( dirname( __FILE__ ) )."/lang" );
}
...
function lptw_recent_posts_options () {
	add_meta_box(
		...
		__( 'Advanced Recent Posts Options', 'lptw_recent_posts' ),
...
function lptw_recent_posts_options_box_content ( $post ) {
	...
	echo '<p><label class="lptw-checkbox-label" for="post_subtitle"><strong>'.__( 'Post Subtitle', 'lptw_recent_posts' ).':&nbsp;</strong></label></p>';
	echo '<p><input class="text" type="text" id="post_subtitle" name="post_subtitle" value="'.esc_attr($post_subtitle).'" /></p>';
	echo '<p><label class="lptw-checkbox-label" for="featured_post"><input class="checkbox" type="checkbox" '.checked( $featured_post, 'on', false ).' id="featured_post" name="featured_post" />&nbsp;'.__( 'Featured post', 'lptw_recent_posts' ).'</label></p>';
	echo '<p class="description">'.__( 'Featured post displays larger than the other posts in Responsive Grid Layout', 'lptw_recent_posts' ).'</p>';
	echo '<div id="lptw-embedded-video-settings">';
	echo '<p><label class="lptw-checkbox-label" for="embedded_video"><input class="checkbox" type="checkbox" '.checked( $embedded_video, 'on', false ).' id="embedded_video" name="embedded_video" />&nbsp;'.__( 'Use embedded video (Experimental feature!!! It may not work properly.)', 'lptw_recent_posts' ).'</label></p>';
	echo '<p><label class="lptw-checkbox-label" for="hide_youtube_controls"><input class="checkbox" type="checkbox" '.checked( $hide_youtube_controls, 'on', false ).' id="hide_youtube_controls" name="hide_youtube_controls" />&nbsp;'.__( 'Hide Youtube player controls', 'lptw_recent_posts' ).'</label></p>';
	echo '<p class="description">'.__( 'Use embedded video (first movie) instead of the Post Featured Image in Responsive Grid Layout. If you have any ideas about this feature or it work not properly, please write me in special topic on <a href="https://wordpress.org/support/topic/new-feature-in-0613-the-embedded-video-instead-of-the-post-featured-image" target="_blank">Support Forum</a>', 'lptw_recent_posts' ).'</p>';
	echo '</div>';
}
...
function lptw_display_recent_posts ( $atts ) {
	...
	if( $lptw_shortcode_query->have_posts() ) {
		...
		$content = __( 'No recent posts', 'lptw_recent_posts' );
	}
	wp_reset_postdata();
	return $content;
}
